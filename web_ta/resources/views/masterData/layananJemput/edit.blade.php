        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-3">
                <h1 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                    <i class="ri-add-circle-line text-blue-600 text-2xl"></i> Edit Data Layanan Jemput
                </h1>
            </div>

            <!-- Form -->
            <form action="{{ route('Layanan-Jemput-Update', $layananJemput->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('put')
                ...
                <div>
                    ...
                    <input type="text" name="ongkir_per_jarak" id="ongkir_per_jarak"
                        class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required value="{{ old('ongkir_per_jarak', $layananJemput->ongkir_per_jarak ?? '') }}">
                </div>
