        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-3">
                <h1 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                    <i class="ri-add-circle-line text-blue-600 text-2xl"></i> Edit Data Sampah
                </h1>
            </div>

            <!-- Form -->
            <form action="{{ route('Jenis-Sampah-Update', $jenis_sampah->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                <div>
                    ...
                    <input type="text" name="nama_sampah" id="nama_sampah"
                        value="{{ old('nama_sampah', $jenis_sampah->nama_sampah) }}"
                ...
                <div>
                    ...
                    <input type="text" name="harga_per_satuan" id="harga_per_satuan"
                        value="{{ old('harga_per_satuan', $jenis_sampah->harga_per_satuan) }}"
                        class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="warna_indikasi" id="warna_indikasi"
                        value="{{ old('warna_indikasi', $jenis_sampah->warna_indikasi) }}" placeholder="#00FF00"
                        class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required>
                </div>
